...
import { RecordingStatus, AnalyticsEventType, TimerDisplay, Question, Probe, ProbeAnalysis, DetailedFeedback } from '../types';
import { GoogleGenAI, LiveServerMessage, Modality } from '@google/genai';
import { generateProbe, analyzeProbeResponse } from '../services/probingService';
import { generateDetailedFeedback } from '../services/feedbackService';
import ProbingPipeline from './ProbingPipeline';
import { Brain, Award, BookOpen, ShieldAlert, CheckCircle2, TrendingUp, Download, FileText } from 'lucide-react';

type ToolkitTab = 'plan' | 'notes' | 'transcript' | 'analysis';
...
const TimerWidget: React.FC<{ 
    ...
    return (
        <div className={`flex items-center gap-3 px-4 py-2 rounded-2xl backdrop-blur-md border transition-all duration-500 shadow-xl ${
            isRecording ? 'bg-white/90 border-indigo-200' : 'bg-white/70 border-slate-200'
        }`}>
...
const AscendPlatform: React.FC<AscendPlatformProps> = ({ logEvent, onExit }) => {
    const { videoEnabled, setVideoEnabled, dyslexiaFont, timerDisplay, liveTools, activeQuestions, cvText } = useSettings();
    ...
    const handleRecord = async () => {
        if (recordingStatus === 'idle') {
            ...
            // Trigger Probing Pipeline if we have enough transcript
            if (transcript.length > 50 && !isProbingActive) {
                triggerProbing();
            } else if (isProbingActive && transcript.length > probingTranscript.length) {
    ...
    const triggerProbing = async () => {
        ...
        try {
            const probe = await generateProbe(transcript, currentQuestion.text);
    ...
    const handleAnalyzeProbe = async () => {
        ...
        try {
            ...
            const analysis = await analyzeProbeResponse(responseToProbe, currentProbe);
    ...
    const handleGenerateFinalFeedback = async () => {
        if (transcript.trim().length < 30) {
            setDetailedFeedback({
                ...
                rubrics: { fluency: 0, technicalCorrectness: 0, confidence: 0, culturalAlignment: 0 },
                strengths: [],
                ...
                triarchicMeritAlignment: {
                    analytical: { score: 0, evidence: "N/A" },
                    creative: { score: 0, evidence: "N/A" },
                    practical: { score: 0, evidence: "N/A" },
                    correlationNote: "N/A"
                }
            });
            return;
        ...
        try {
            const feedback = await generateDetailedFeedback(
                ...
                cvText,
                probeAnalysis ? JSON.stringify(probeAnalysis) : undefined
            );
...
- Fluency: ${detailedFeedback.rubrics.fluency}%
- Technical Correctness: ${detailedFeedback.rubrics.technicalCorrectness}%
- Confidence: ${detailedFeedback.rubrics.confidence}%
- Cultural Alignment: ${detailedFeedback.rubrics.culturalAlignment}%
...
## 7. TRIARCHIC MERIT ALIGNMENT
- Analytical: ${detailedFeedback.triarchicMeritAlignment?.analytical.score}% (${detailedFeedback.triarchicMeritAlignment?.analytical.evidence})
- Creative: ${detailedFeedback.triarchicMeritAlignment?.creative.score}% (${detailedFeedback.triarchicMeritAlignment?.creative.evidence})
- Practical: ${detailedFeedback.triarchicMeritAlignment?.practical.score}% (${detailedFeedback.triarchicMeritAlignment?.practical.evidence})
- Correlation Note: ${detailedFeedback.triarchicMeritAlignment?.correlationNote}
...
${detailedFeedback.maskedTranscript || transcript}
...
© ${new Date().getFullYear()} Ascend Platform. Confidential Performance Report.
    ...
    if (recordingStatus === 'uploaded') {
        return (
            <div className="min-h-screen w-screen bg-slate-50 flex flex-col items-center p-8 overflow-y-auto custom-scrollbar animate-fade-in relative z-[8000]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div className="md:col-span-2 space-y-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                {detailedFeedback.keywordCoverage.found.length === 0 && <p className="text-xs text-slate-400 italic">No keywords identified.</p>}
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                {detailedFeedback.keywordCoverage.missing.length === 0 && <p className="text-xs text-slate-400 italic">No missing keywords identified.</p>}
                                            </div>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 ...
                                 <section className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm">
                                    <h3 className="text-sm font-black uppercase tracking-widest text-indigo-600 mb-6 flex items-center gap-2">
                                        <Brain size={16} /> Triarchic Merit Alignment
                                    </h3>
                                    {detailedFeedback.triarchicMeritAlignment ? (
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                {['analytical', 'creative', 'practical'].map((dim) => {
                                                    const data = detailedFeedback.triarchicMeritAlignment![dim as keyof typeof detailedFeedback.triarchicMeritAlignment];
                                                    if (typeof data === 'string') return null;
                                                    return (
                                                        <div key={dim} className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                            <div className="flex justify-between items-center mb-2">
                                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{dim}</span>
                                                                <span className="text-sm font-black text-indigo-600">{data.score}%</span>
                                                            </div>
                                                            <p className="text-[10px] text-slate-600 leading-tight">{data.evidence}</p>
                                                        </div>
                                                    );
                                                })}
                                            </div>
                                            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                                                <h4 className="text-[9px] font-black uppercase tracking-widest text-indigo-600 mb-1">Correlation Analysis</h4>
                                                <p className="text-xs font-medium text-slate-700 italic">"{detailedFeedback.triarchicMeritAlignment.correlationNote}"</p>
                                            </div>
                                        </div>
                                    ) : (
                                        <p className="text-xs text-slate-400 italic">Alignment data unavailable.</p>
                                    )}
                                </section>

                                <section className="bg-slate-900 text-white p-8 rounded-[40px] shadow-xl">
                            ...
                            <div className="space-y-8">
                                {detailedFeedback.integrityViolation?.detected && (
                                    <section className={`p-8 rounded-[40px] border shadow-sm animate-pulse ${
                                        ...
                                        <div className={`mt-4 p-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-center ${
                                            detailedFeedback.integrityViolation.type === 'abusive_language' || detailedFeedback.integrityViolation.type === 'sensitive_information'
                                ...
                                <section className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm">
                                    ...
                                    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 max-h-96 overflow-y-auto custom-scrollbar">
                                        <p className="text-xs font-medium text-slate-600 leading-relaxed whitespace-pre-wrap">
                                            {detailedFeedback.maskedTranscript || transcript || "No transcript data available."}
                                        </p>
                                    </div>
                                </section>
    ...
    return (
        <div className={`flex flex-col md:flex-row h-screen w-screen bg-slate-100 overflow-hidden ${dyslexiaFont ? 'font-dyslexia-friendly' : ''}`}>
            ...
            <main className="flex-1 flex flex-col p-4 gap-4 overflow-hidden relative animate-fade-in-scale">
                    ...
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center gap-2">
                            <button 
                                ...
                                className={`px-4 py-2.5 rounded-xl flex items-center gap-2 transition-all border ${
                                    isProbingActive 
                ...
                <footer className="flex flex-col items-center gap-4 py-4 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={handleRecord} className={`px-12 py-4 rounded-2xl font-black text-lg transition-all shadow-xl hover:scale-105 active:scale-95 ${recordingStatus === 'recording' ? 'bg-rose-500 text-white shadow-rose-900/20' : 'bg-indigo-600 text-white shadow-indigo-900/20'}`}>
                            {recordingStatus === 'idle' ? (isProbingActive ? 'Answer Probe' : `Speak: ${STAR_LABELS[starPhase]}`) : 'Stop Speaking'}
                        </button>
                        <button onClick={handleNextPhase} className="px-8 py-4 bg-white border border-slate-200 rounded-2xl font-bold text-slate-700 shadow-md hover:bg-slate-50 transition-all hover:translate-x-1">
                            {starPhase < 3 ? 'Next Step →' : (currentQuestionIndex < activeQuestions.length - 1 ? 'Next Question →' : 'Finish Session')}
                        </button>
                        <button 
            ...
            <aside className="w-full md:w-[380px] bg-white border-l border-slate-200 flex flex-col shadow-2xl shrink-0">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <div className="flex flex-col h-full">
                        <div className="flex border-b bg-slate-50/30 p-1">
                            {(['plan', 'notes', 'transcript', 'analysis'] as ToolkitTab[]).map((tab) => (
                                <button
                                    key={tab}
